...
train_weights = train[['prob']].to_numpy()

val_triples = val[['head', 'variable', 'value']].to_numpy()
val_weights = val[['prob']].to_numpy()

test_triples = test[['head', 'variable', 'value']].to_numpy()
test_weights = test[['prob']].to_numpy()
...
def train_model(modelname, epochs, batches_count, k, structural_wt, lr, entities=entities, relations=relations):
    ...
    if(modelname=="transE_with_weights"):        
    ...
    elif(modelname=="holE_with_weights"):
    ...
    elif(modelname=='transE_without_weights'):
        model = TransE(batches_count=batches_count, seed=555, epochs=epochs, k=k, eta=5, loss='pairwise', loss_params={'margin':1}, regularizer='LP', regularizer_params={'lambda':0.1}, verbose=True, embedding_model_params={'structural_wt':structural_wt}, optimizer_params = {'lr':lr})
        losses_list= model.fit(train_triples)
        path = 'embeddings_final/transE_without_weights/'
    elif(modelname=='holE_without_weights'):
        model = HolE(batches_count=batches_count, seed=555, epochs=epochs, k=k, eta=5, loss='pairwise', loss_params={'margin':1}, regularizer='LP', regularizer_params={'lambda':0.1}, verbose=True, embedding_model_params={'structural_wt':structural_wt}, optimizer_params = {'lr':lr})
        losses_list= model.fit(train_triples)
        path = 'embeddings_final/holE_without_weights/'
...
lrs=[0.1, 0.01]           #Learning Rate
embed_size=[40, 50]             #Embedding size k
epochs=[50, 100]         #Epochs
model_name=['transE_with_weights', 'holE_with_weights', 'transE_without_weights', 'holE_without_weights']   #ModelNames
