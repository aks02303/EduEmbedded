...
class EmbeddingModel(abc.ABC):
    ...
    def _transfer(self, e, n):
        """Required only for TransH.
        Transfer function to transfer entity embeddings from normal vector of planes 

        Parameters
        ----------
        e : Tensor, shape [n]
            The embeddings of a list of entities.
        n : Tensor, shape [n]
            normal vectors of planes to transfer entity embeddings.


        Returns
        -------
        transferred entity embeddings : TensorFlow operation
            The operation corresponding to the transfer function.

        """
        logger.error('_transfer is a placeholder function in an abstract class')
        NotImplementedError("This function is a placeholder in an abstract class")
    ...
    def _initialize_parameters(self):
        #Substitute embedding_def function If using OpenKE for parallel TransH implementation  
        ...
        if not self.dealing_with_large_graphs:
            print("In init not large graphs")
            ...
            self.rel_emb = tf.get_variable('rel_emb_{}'.format(timestamp),
                                           ...
                                           dtype=tf.float32)
            #self.rel_emb1 = tf.placeholder(tf.int32, [None])
            self.normal_vectors = tf.get_variable('normal_vectors_{}'.format(timestamp), 
                                                  shape=[len(self.rel_to_idx), self.internal_k], 
                                                  initializer=self.initializer.get_relation_initializer(
                                               len(self.rel_to_idx), self.internal_k))
        else:
            ...
            self.rel_emb = tf.get_variable('rel_emb_{}'.format(timestamp),
                                           ...
                                           initializer=self.initializer.get_relation_initializer(
                                               len(self.rel_to_idx), self.internal_k),
                                           dtype=tf.float32)
            self.normal_vectors = tf.get_variable(name = "normal_vectors", shape=[len(self.rel_to_idx), self.internal_k], initializer=self.initializer.get_relation_initializer(
                                               len(self.rel_to_idx), self.internal_k),
                                           dtype=tf.float32)
    ...
    def _get_model_loss(self, dataset_iterator):
        ...
        with tf.control_dependencies(dependencies):
            print("In loss fn run dependencies")
            entities_size = 0
            entities_list = None
            ...
            rr = self._entity_lookup(x_pos[:, 2])
            # Starting changes for TransH
            #Calculating score functions for all positive triples
            #Getting the required normal vectors of planes to transfer entity embeddings
            print(self.rel_emb)
            #pos_norm = tf.nn.embedding_lookup(self.normal_vectors, self.rel_emb)
            pos_norm = tf.nn.embedding_lookup(self.normal_vectors, x_pos[:, 1])
            print(self.normal_vectors)
            #pos_norm = e_p_pos
            e_s_pos = self._transfer(e_s_pos, pos_norm)
            print(e_s_pos)
            print(e_s_pos.shape)
            e_o_pos = self._transfer(e_o_pos, pos_norm)
            e_p_pos = e_p_pos

            # Ending changes for TransH
            ...
            if self.use_focusE:
                ...
                print("====")
                print(scores_pos.shape)
                print(scores_pos)
                print(weights_pos)
                print(weights_neg)
                print(weights_pos.shape)
            ...
            for side in corruption_sides:
                ...
                # Starting changes for TransH
                #Calculating score functions for all negative triples
                #Getting the required normal vectors of planes to transfer entity embeddings
                #neg_norm = tf.nn.embedding_lookup(self.normal_vectors, e_p_neg)
                neg_norm = tf.nn.embedding_lookup(self.normal_vectors, x_neg_tf[:, 1])
                e_s_neg = self._transfer(e_s_neg, neg_norm)
                e_o_neg = self._transfer(e_o_neg, neg_norm)
                e_p_neg = e_p_neg
                #TransH changes ends
                ...
                if self.use_focusE:
                    print("=====In corruption part")
                    print(scores_neg.shape)
                    print(scores_neg)
                    print(weights_pos)
                    print(weights_neg)
                    print(weights_neg.shape)
 ...
 https://docs.ampligraph.org/en/latest/dev_notes.html#dealing-with-large-graphs")
    ...
    def predict(self, X, from_idx=False):
        ...
        if not self.dealing_with_large_graphs:
            ...
            #Changes for TransH starts
            predict_norm = tf.nn.embedding_lookup(self.normal_vectors, e_p)
            e_s = self._transfer(e_s, predict_norm)
            e_o = self._transfer(e_o, predict_norm)
            r_e = e_p
            #Changes for TransH ends
