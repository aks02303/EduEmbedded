...
class EmbeddingModel(abc.ABC):
    ...
    def _get_model_loss(self, dataset_iterator):
        ...
        with tf.control_dependencies(dependencies):
            ...
            #e_s_pos = tf.Print(e_s_pos, [e_s_pos], message="This is e_s_pos: ",summarize=-1)
            #e_p_pos = tf.Print(e_p_pos, [e_p_pos], message="This is e_p_pos: ", summarize=-1)
            #e_o_pos = tf.Print(e_o_pos, [e_o_pos], message="This is e_o_pos: ", summarize=-1)
            ...
            if self.use_focusE:
                ...
                #sess = tf.InteractiveSession()
                #scores_pos = tf.Print(scores_pos, [scores_pos], message="This is scores_pos: ",summarize=-1)
                #weights_pos = tf.Print(weights_pos, [weights_pos], message="This is weights_pos: ", summarize=-1)
                #weights_neg = tf.Print(weights_neg, [weights_neg], message="This is weights_neg: ", summarize=-1)
                #print("============")
 ...
 https://docs.ampligraph.org/en/latest/dev_notes.html#dealing-with-large-graphs")
            ...
            for epoch in epoch_iterator_with_progress:
                #epoch_i = epoch_i + 1
                #print("Epoch Running issss: ",epoch_i)
                losses = []
