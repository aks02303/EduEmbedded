...
#
from .EmbeddingModel_3 import EmbeddingModel, register_model
...
class TransH(EmbeddingModel):
    ...
    def _fn(self, e_s, e_p, e_o):
        ...
        """
        #e_s = tf.nn.l2_normalize(e_s, -1)
        #e_o = tf.nn.l2_normalize(e_o, -1)
        #e_p = tf.nn.l2_normalize(e_p, -1)
        #return abs(e_s + e_p - e_o)
        #print(constants.DEFAULT_NORM_TRANSE)
        #print(constants.DEFAULT_NORM_TRANSH)

        return tf.negative(
            tf.norm(e_s + e_p - e_o, ord=self.embedding_model_params.get('norm', 2),
                    axis=1))
